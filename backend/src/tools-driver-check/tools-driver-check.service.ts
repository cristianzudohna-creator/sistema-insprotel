...
@Injectable()
export class ToolsDriverCheckService {
  ...
  private async canAccess(currentUser: any, id: number) {
    ...
    const check = await this.prisma.toolsDriverCheck.findUnique({
      where: { id },
      include: {
        items: true,
        user: true,
    ...
    if (!check) {
      throw new NotFoundException("Check list conductor no encontrado");
    }
  ...
  async create(
    ...
    const lastCheck = await this.prisma.toolsDriverCheck.findFirst({
      orderBy: { id: "desc" },
    });

    const nextNumber = (lastCheck?.id || 0) + 1;

    const generatedFolio = `COND-${new Date().getFullYear()}-${String(
      nextNumber,
    ).padStart(4, "0")}`;

    return this.prisma.toolsDriverCheck.create({
      data: {
        folio: generatedFolio,
        date: new Date(),
        contract: data.contract || null,
        driverName: data.driverName || user.name || null,
        mobile: data.mobile || null,
        supervisorInspectorName: data.supervisorInspectorName || null,
        zone: data.zone || null,
        heightExamExpiration: data.heightExamExpiration || null,

        driverSignatureUrl: driverSignature
          ? `/uploads/tools-driver-check/${driverSignature.filename}`
          : null,

        inspectorSignatureUrl: inspectorSignature
          ? `/uploads/tools-driver-check/${inspectorSignature.filename}`
          : null,
  ...
  async remove(currentUser: any, id: number) {
    ...
    return this.prisma.toolsDriverCheck.delete({
      where: { id },
    });
  }
  ...
  async generatePdf(currentUser: any, id: number, res: Response) {
    ...
    const drawStatus = (value: any, target: string) => {
      return String(value || "").toUpperCase() === target ? "X" : " ";
    };
    ...
    drawCell(margin, y, 95, rowH, "Conductor", {
      fill: gray,
      bold: true,
    });
    drawCell(margin + 95, y, leftW - 95, rowH, this.text(check.driverName));
    ...
    drawCell(
      ...
      leftW - 95,
      rowH,
      this.text(check.heightExamExpiration),
    );

    drawCell(margin + leftW, y, 95, rowH, "Folio", {
      fill: gray,
      bold: true,
    });
    drawCell(margin + leftW + 95, y, leftW - 95, rowH, this.text(check.folio));

    y += rowH + 10;

    drawCell(margin, y, contentWidth, 18, "ELEMENTOS Y DOCUMENTACIÓN", {
      fill: blue,
      bold: true,
      align: "center",
      fontSize: 8,
    ...
    const colQty = 50;
    const colStatus = 38;
    const colObs = contentWidth - colN - colName - colQty - colStatus * 3;
    ...
    const drawTableHeader = () => {
      ...
      drawCell(margin + colN, y, colName, 18, "Elemento", {
        fill: blue,
        bold: true,
        align: "center",
        fontSize: 6,
      ...
      drawCell(margin + colN + colName, y, colQty, 18, "Cant.", {
        fill: blue,
        bold: true,
        align: "center",
        fontSize: 6,
      ...
      drawCell(margin + colN + colName + colQty, y, colStatus, 18, "Bueno", {
        fill: blue,
        bold: true,
        align: "center",
        fontSize: 5,
    ...
    drawSignature(margin, y + 18, signW, signH, check.driverSignatureUrl);

    doc
      .font("Helvetica-Bold")
      .fontSize(8)
      .fillColor(black)
      .text(this.text(check.driverName), margin, y + signH - 2, {
        width: signW,
        align: "center",
      });
